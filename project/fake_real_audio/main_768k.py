...
#shape2=76
#shape2 = 69
tmp_f = '../mel_pictures/train/768k_real/FDD33F5E_9.jpg'
...
#### finish loading
for i in range(len(speaker_768k)):
    label_to_dict[speaker_768k[i]] = i


#train_256k_real_wavs = librosa.util.find_files('../eda/train/256k/',recurse=False)


data_paths = glob('../mel_pictures/train/768k_real/*')
...
test_paths = glob('../mel_pictures/test/768k/*')
...
checkpointer = ModelCheckpoint(filepath='../h5s/768k_best_resnet34_iaa_255_shuffle.h5', monitor='val_acc',mode='max',verbose=1,save_best_only=True)
stop = EarlyStopping(monitor='val_acc', patience=8, verbose=1, mode='max')
...
model.load_weights("../h5s/768k_best_resnet34_iaa_255_shuffle.h5")
...
for i in range(len(id_)):
    pred_768k_dict[id_[i]] = label_string[i]

with open('../jsons/pred_768k_dict.json','w') as f:
    json.dump(pred_768k_dict,f)
...
win = 2
plt.savefig('../history_plots/history{}_768k.png'.format(win))
