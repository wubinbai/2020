...
FEATURE_IMPORTANCES_LOW = np.array([ 37,  42,  55,  56,  58,  71,  75,  76,  77,  80,  82,  83,  84,
        85,  87,  90,  91,  92,  93,  98,  99, 100, 101, 102, 104, 106,
       107, 108, 109, 113, 114, 115, 116, 117, 134, 135, 136, 137, 138,
       139, 140, 141, 143, 144, 149, 151, 152, 157])
FEATURE_IMPORTANCES_LOW = np.array([ 25,  30,  37,  42,  49,  55,  56,  58,  71,  75,  76,  77,  80,
        82,  83,  84,  85,  87,  90,  91,  92,  93,  98,  99, 100, 101,
       102, 104, 106, 107, 108, 109, 113, 114, 115, 116, 117, 129, 130,
       134, 135, 136, 137, 138, 139, 140, 141, 143, 144, 147, 149, 151,
       152, 153, 156, 157])


FEATURE_IMPORTANCES_LOW = np.array([ 5,   6,  12,  13,  15,  21,  22,  23,  25,  29,  30,  32,  33,
        37,  39,  40,  42,  43,  46,  49,  52,  54,  55,  56,  58,  60,
        63,  67,  71,  75,  76,  77,  80,  82,  83,  84,  85,  86,  87,
        90,  91,  92,  93,  94,  97,  98,  99, 100, 101, 102, 103, 104,
       106, 107, 108, 109, 110, 113, 114, 115, 116, 117, 118, 122, 126,
       127, 128, 129, 130, 134, 135, 136, 137, 138, 139, 140, 141, 143,
       144, 145, 146, 147, 149, 151, 152, 153, 154, 155, 156, 157])
...
def train_and_eval(n_feats_now, to_save='n'):
    ...
    scores = []
    # remove bad feats
    X.iloc[:,FEATURE_IMPORTANCES_LOW] = 0
    ...
    max_ind = np.argmax(model.feature_importances_)
    print(max_ind)
    thresholds = sort(model.feature_importances_)
    print('thres', thresholds)
    features_score = model.feature_importances_
    print(type(features_score))
    locations_small = np.where(features_score<0.00625)
    print('len(features_score): ',len(features_score))
    print('np.where(features_score<0.00625): ', locations_small)
    plt.figure()
...
def save_data(n_feats_now):
    ...
    NUM_FUNCTIONS = 21
    array_all_feats = np.zeros((1000,NUM_FUNCTIONS*8-2+2))
